...
#            loss_r = ...
#            loss = ...
            delta_pred, reward_pred = self.model(states, a_onehot)
            target_delta = next_states - states
            loss_s = nn.functional.mse_loss(
                delta_pred,
                target_delta,
            )
...
#            reward_mae = ...
            ...
            delta_pred, reward_pred = self.model(
                states,
                a_onehot,
            )

            next_state_pred = states + delta_pred
            state_mse = nn.functional.mse_loss(
                next_state_pred,
                next_states,
            ).item()
            ...
            state_mae = nn.functional.l1_loss(
                next_state_pred,
                next_states,
            ).item()

            reward_mae = nn.functional.l1_loss(
                reward_pred,
                rewards,
            ).item()
